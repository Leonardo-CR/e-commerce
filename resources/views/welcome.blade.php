...
<body class="antialiased text-slate-900 selection:bg-indigo-100 selection:text-indigo-900">
    ...
    <nav x-data="{ mobileMenuOpen: false }" class="fixed w-full z-50 glass-nav">
                ...
                <div class="flex items-center space-x-2 sm:space-x-6">
                    @auth
                        ...
                        <div class="hidden md:flex items-center space-x-4 border-l border-slate-200 pl-6 ml-2">
                            ...
                            <button x-on:click="$dispatch('show-logout-modal')"
                    ...
                    @else
                        <div class="hidden sm:flex items-center space-x-4">
                            ...
                            <a href="{{ route('register') }}"
                                class="bg-indigo-600 text-white px-5 py-2.5 rounded-full text-sm font-bold shadow-lg shadow-indigo-200">Registrarse</a>
                        </div>
                    @endauth

                    <!-- Mobile Menu Button -->
                    <button @click="mobileMenuOpen = !mobileMenuOpen"
                        class="lg:hidden p-2 text-slate-600 hover:bg-slate-50 rounded-xl transition">
        ...
        <div x-show="mobileMenuOpen" x-transition:enter="transition ease-out duration-200"
            ...
            class="lg:hidden bg-white border-t border-slate-100 shadow-xl" x-cloak>
            <div class="px-6 py-8 space-y-6">
                ...
                <div class="pt-6 border-t border-slate-50 flex flex-col space-y-4">
                    @auth
                        ...
                        <button x-on:click="$dispatch('show-logout-modal')"
                            class="text-left text-base font-bold text-red-500">Salir</button>
                    @else
                        <a href="{{ route('login') }}" class="text-base font-bold text-slate-600">Iniciar Sesión</a>
                        <a href="{{ route('register') }}"
                            class="inline-flex justify-center bg-indigo-600 text-white px-6 py-3 rounded-full text-base font-bold">Registrarse</a>
                    @endauth
                </div>
            </div>
        </div>
    ...
    <section class="relative min-h-screen flex items-center pt-20 overflow-hidden hero-gradient">
        <div class="max-w-7xl mx-auto px-6 sm:px-8 grid lg:grid-cols-2 gap-12 items-center">
            <div class="relative z-10">
                <span
                    class="inline-block py-1 px-3 rounded-full bg-indigo-50 text-indigo-600 text-xs font-bold uppercase tracking-wider mb-6">Nueva
                    Serie 2024</span>
                <h1 class="text-5xl md:text-6xl lg:text-7xl font-extrabold text-slate-900 leading-[1.1] mb-6">
                    Redefiniendo la <br>
                    <span
                        class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-indigo-400">Perfección
                        Sonora</span>
                </h1>
                <p class="text-lg text-slate-500 mb-10 max-w-lg leading-relaxed">
                    Eleva tu experiencia auditiva con acústica de vanguardia y diseño ergonómico. Diseñado para quienes
                    notan la diferencia.
                </p>
                ...
                <div class="mt-12 flex items-center space-x-6">
                    ...
                    <p class="text-sm text-slate-500"><span class="font-bold text-slate-900">+2,400</span> reseñas
                        verificadas</p>
                </div>
            </div>
            ...
            <div class="relative">
                ...
                <img src="{{ asset('images/hero.png') }}" alt="Premium Headphones"
                    class="relative z-10 w-full h-auto drop-shadow-2xl floating">
            </div>
        </div>
    </section>
    ...
    <section id="products" class="py-24">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
                <div>
                    <h2 class="text-3xl md:text-5xl font-extrabold text-slate-900 mb-4">Sonido Curado <br> Para Tu
                        Estilo de Vida</h2>
            ...
            @if(isset($products) && $products->count() > 0)
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
                    @foreach($products as $product)
                                    ...
                                    @endphp
                          <div
                                        class="product-card group flex flex-col h-full bg-white rounded-[2.5rem] p-4 border border-slate-100 shadow-sm relative overflow-hidden">
                                        <div
                                            class="relative aspect-square rounded-[2rem] overflow-hidden bg-slate-50 flex items-center justify-center mb-6">
                                            <img src="{{ $imageUrl }}" alt="{{ $product->name }}"
                                                class="w-4/5 h-4/5 object-contain group-hover:scale-110 transition-transform duration-500">
                                            <div class="absolute inset-0 bg-indigo-600/0 group-hover:bg-indigo-600/5 transition-colors">
                                            </div>
                                        </div>
                                        ...
                                        <div class="mt-auto flex items-center justify-between pt-4 border-t border-slate-50">
                                            ...
                                            <a href="/headphones"
                                                class="bg-indigo-600 p-3 rounded-xl shadow-lg shadow-indigo-100 text-white hover:bg-slate-900 transition-all transform active:scale-95">
                ...
                <div class="mt-16 text-center">
                    <a href="/headphones"
                        class="inline-flex items-center space-x-3 bg-slate-900 text-white px-8 py-4 rounded-2xl font-bold hover:scale-105 transition-all shadow-xl shadow-slate-200 group">
    ...
    <section class="py-24 px-6">
        <div
            class="max-w-7xl mx-auto rounded-[3rem] overflow-hidden relative bg-slate-900 min-h-[500px] flex items-center">
            <img src="{{ asset('images/lifestyle.png') }}"
                class="absolute inset-0 w-full h-full object-cover opacity-60" alt="Lifestyle">
            <div class="relative z-10 p-8 md:p-16 max-w-2xl">
                <h2 class="text-4xl md:text-5xl font-extrabold text-white mb-6">Diseñado para el <br> Silencio Perfecto
                </h2>
    ...
    <footer class="bg-white pt-24 pb-12 border-t border-slate-100">
        <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            <div class="md:col-span-2">
                ...
                <p class="text-slate-500 max-w-sm mb-8 leading-relaxed">
                    Forjando el futuro del sonido a través de la innovación, la sustentabilidad y una calidad sin
                    compromisos.
                </p>
                <div class="flex space-x-4">
                    <a href="#"
                        class="w-10 h-10 rounded-full border border-slate-100 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:border-indigo-600 transition">
        ...
        <div
            class="max-w-7xl mx-auto px-6 pt-8 border-t border-slate-50 flex flex-col md:flex-row justify-between items-center gap-4">
